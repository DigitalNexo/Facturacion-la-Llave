...
export default function EditInvoicePage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!invoiceRes.ok) throw new Error('Error al cargar factura');
      ...
      if (inv.status !== 'draft') {
        showError('Esta factura no se puede editar');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Error al actualizar factura');
      }
    ...
    <div className="max-w-5xl mx-auto p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          Borrador - Solo editable antes de emitir
        </p>
      </div>
